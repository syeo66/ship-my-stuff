...
let Web3Connector = {
  ...
  init: () => {
    return new Promise((resolve, reject) => {
      ...
      Web3Connector.accountUpdateStream.subscribe(account => {
  ...
  reconnect: async _ => {
    ...
    Web3Connector.ipfs = ipfsAPI(
      ipfsUrl.hostname || "localhost",
      ipfsUrl.port || "5001",
      { protocol: ipfsUrl.protocol.replace(":", "") || "http" }
    );
      ...
      Web3Connector.providerStream = fromEvent(window, "message").pipe(
        mergeMap(({ data }) => {
          if (data && data.type && data.type === "ETHEREUM_PROVIDER_SUCCESS") {
            ...
            Web3Connector.web3 = new Web3(Web3Connector.web3Provider);
            if (!!Web3Connector.connectionData.account) {
              Web3Connector.web3.eth.defaultAccount =
                Web3Connector.connectionData.account;
              Web3Connector.account.account =
                Web3Connector.connectionData.account;
            }
          }
  ...
  readyContracts: () => {
    return new Promise((resolve, reject) => {
      if (
        typeof Web3Connector.contracts.TransportMarket != "undefined" ||
  ...
  initAccountUpdateStream: () => {
    Web3Connector.accountUpdateStream = Web3Connector.timerStream
      .pipe(map(v => Math.round(v / 5)))
      ...
      .pipe(
        mergeMap(v =>
          Observable.create(observable => {
            ...
            Web3Connector.getWeb3().eth.getBalance(
              account,
              async (error, v) => {
                observable.next({
                  account: account,
  ...
  initNetworkStatusStream: () => {
    Web3Connector.networkStatusStream = Web3Connector.timerStream
      .pipe(
        mergeMap(v =>
  ...
  initContracts: () => {
    return new Promise(async (resolve, reject) => {
      ...
      Web3Connector.contracts.TransportMarket = TruffleContract(
        TransportMarketArtifact
      );
      Web3Connector.contracts.TransportMarket.setProvider(
        Web3Connector.web3Provider
      );
      resolve();
    });
  ...
  createParcel: (confirmationHash, description, price) => {
    return new Promise((resolve, reject) => {
      ...
      Web3Connector.contracts.TransportMarket.deployed()
        .then(contract => {
          try {
            contract
              .createParcel(confirmationHash, description, {
  ...
  getParcelIndex: hash => {
    return new Promise((resolve, reject) => {
      ...
      Web3Connector.contracts.TransportMarket.deployed()
        .then(contract => {
          try {
            contract.getParcelIndex
              .call(hash, { from: Web3Connector.account.account })
              .then(data => {
                resolve(data);
              })
              .catch(err => reject(err));
  ...
  getParcel: hash => {
    return new Promise(async (resolve, reject) => {
      ...
      Web3Connector.contracts.TransportMarket.deployed()
        .then(async contract => {
          try {
            const data = await contract.getParcelDescription.call(index, {
              from: Web3Connector.account.account
            });
            const description = JSON.parse(data);

            // [senderAddress, haulerAddress, price, state]
            const parcel = await contract.getParcel.call(index, {
              from: Web3Connector.account.account
            });
            ...
            Web3Connector.ipfs.files
              ...
              .then(files => {
                ...
                resolve({
                  ...
                  fromAddress: Web3Connector.safeString(
                    parcelContent.fromAddress
                  ),
                  from3Words: Web3Connector.safeString(
                    parcelContent.from3Words
                  ),
      ...
      Web3Connector.contracts.TransportMarket.deployed()
        .then(contract => {
          try {
            ...
            method
              .call({ from: Web3Connector.account.account })
              .then(data => {
                let parcels = [];
